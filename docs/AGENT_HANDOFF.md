# ARCHIVE: история проекта диплома до актуального code audit

> **Не использовать как источник текущего состояния.** Актуальный вход: [START_HERE.md](START_HERE.md), затем `experiments/registry.yaml` и узкий reproduction-документ. Ниже сохранены хронология, обзор литературы и контекст презентации; июньские пути, инфраструктура, статусы и UR4Rec-метрики могут быть устаревшими. Legacy Avito `0.9417` невалиден; прежний pseudo-profile `0.3413` также признан all-tie schema artifact. History `brand`/`model_name` не являются автомобильными attrs — см. [актуальный аудит Avito](avito_preferences.md). Corrected-v3 завершён, актуальные метрики находятся в START_HERE/registry.

> **Назначение файла:** историческая справка для восстановления решений и обзора статей.
> **Для нового агента:** сначала `@docs/START_HERE.md`; этот файл читать только для хронологии/литературы.
...
*Архивный срез: май–июнь 2026. Текущие эксперименты обновлять в `experiments/registry.yaml`, `results/current/manifest.json` и `docs/START_HERE.md`, не в этом файле.*
